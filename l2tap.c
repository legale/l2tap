...
static ssize_t local_ip_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    return sprintf(buf, "%s", local_ip);
}

static ssize_t local_ip_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
    snprintf(local_ip, sizeof(local_ip), "%s", buf);
    return count;
}

static ssize_t remote_ip_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    return sprintf(buf, "%s", remote_ip);
}

static ssize_t remote_ip_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
    snprintf(remote_ip, sizeof(remote_ip), "%s", buf);
    return count;
}

static ssize_t local_port_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    return sprintf(buf, "%d\n", local_port);
}

static ssize_t local_port_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
...
static ssize_t remote_port_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
...
static ssize_t remote_port_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    return sprintf(buf, "%d\n", remote_port);
}

static struct kobj_attribute local_ip_attr = __ATTR(local_ip, 0660, local_ip_show, local_ip_store);
static struct kobj_attribute remote_ip_attr = __ATTR(remote_ip, 0660, remote_ip_show, remote_ip_store);
static struct kobj_attribute local_port_attr = __ATTR(local_port, 0660, local_port_show, local_port_store);
static struct kobj_attribute remote_port_attr = __ATTR(remote_port, 0660, remote_port_show, remote_port_store);
...
static void udp_receive(struct sock *sk)
{
    ...
    while (true) {
        len = kernel_recvmsg(priv->udp_sock, &msg, &iov, 1, MAX_UDP_PAYLOAD, MSG_DONTWAIT | MSG_NOSIGNAL);
        ...
        if (len < sizeof(struct ethhdr)) {
            printk(KERN_ERR "%s: Received packet too small for Ethernet header, len = %d\n", DRV_NAME, len);
            return;
        }
        ...
        printk(KERN_INFO "%s: Raw packet data: %02x %02x %02x %02x %02x %02x %02x %02x\n",
               DRV_NAME, buf[0], buf[1], buf[2], buf[3], buf[4], buf[5], buf[6], buf[7]);

        // Извлечение Ethernet-заголовка
        eth = eth_hdr(skb);
        printk(KERN_INFO "%s: Received packet: Src MAC: %pM, Dest MAC: %pM, Protocol: 0x%x\n",
...
static netdev_tx_t l2tap_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
    struct msghdr msg = {};
    struct kvec iov;
    ...
    printk(KERN_INFO "%s: Sending packet: Src MAC: %pM, Dest MAC: %pM, Protocol: 0x%x\n", 
           DRV_NAME, eth->h_source, eth->h_dest, ntohs(eth->h_proto));
    ...
    printk(KERN_INFO "%s: Before sending packet via UDP. Remote IP: %pI4, Remote Port: %u\n", 
...
static int l2tap_up(struct net_device *dev)
{
    ...
    if (priv->udp_sock) {
        printk(KERN_WARNING "%s: UDP socket already initialized. Releasing existing socket.\n", DRV_NAME);
...
static int l2tap_newlink(struct net *net, struct net_device *dev, struct nlattr *tb[], struct nlattr *data[], struct netlink_ext_ack *extack)
{
    struct l2tap_priv *priv = netdev_priv(dev);
...
static int udp_tunnel_init(struct l2tap_priv *priv)
{
    ...
    ret = priv->udp_sock->ops->bind(priv->udp_sock, (struct sockaddr *)&addr, sizeof(addr));
    if (ret < 0) {
